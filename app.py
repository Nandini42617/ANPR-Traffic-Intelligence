...
if start:
    ...
    if not sources:
        ...
        st.warning(
            "Upload at least one traffic video "
            "or choose a project feed."
        )
...
kpi[2].metric(
    "PLATES READ",
    summary["plate_recognized"],
)

kpi[3].metric(
    "LOW / UNREAD",
    summary["plate_unreadable"],
)
...
with tab1:
    ...
    st.subheader(
        "Cross-camera vehicle trajectory"
    )
    ...
    if query:
        ...
        if route.empty:
            ...
            c1.metric(
                "GLOBAL ID",
                str(
                    route.global_vehicle_id.iloc[0]
                ),
            )
    ...
    for index, result in enumerate(
        ...
        if frame.empty:
            ...
            st.download_button(
                ...
                .encode(),
                (
                    f"{Path(result.input_video).stem}"
                    "_records.csv"
                ),
                "text/csv",
                key=(
...
with d3:
    ...
    st.caption(
        "Global IDs are plate-backed. "
        "UNKNOWN vehicles remain camera-scoped "
        "unless appearance ReID is enabled."
    )
