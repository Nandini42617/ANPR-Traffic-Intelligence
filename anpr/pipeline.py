...
class ANPRPipeline:
    def __init__(
        ...
        self.plate_imgsz = (
            min(settings.plate_image_size, 640)
            if self.device == "cpu"
            else settings.plate_image_size
        )
    ...
    def _crop(frame: np.ndarray, box: np.ndarray):
        ...
        return (
            frame[coords[1] : coords[3], coords[0] : coords[2]],
            coords,
        )
    ...
    def _candidate_cluster(
        ...
        """Match OCR variants conservatively."""

        for other, _ocr, _plate, _quality, _frame in candidates:
            if (
                abs(len(text) - len(other)) <= 1
                and SequenceMatcher(
                    None,
                    text,
                    other,
    ...
    def process_video(
        ...
        plate_observations = defaultdict(list)
        ...
        try:
            while (
                ...
                if frame_no % max(
                    ...
                    result.frames_inferred += 1

                    current = []

                    tracked = self.vehicle_model.track(
                        source=frame,
                        persist=True,
                    ...
                    if (
                        tracked.boxes is not None
                        and len(tracked.boxes)
                    ):
                        ...
                        for box, cls, raw_id in zip(
                            ...
                            vehicle_crop, vehicle_coords = (
                                self._crop(frame, box)
                            )
                            ...
                            should_detect = (
                                ...
                                % max(
                                    1,
                                    self.settings.plate_detection_interval,
                                )
                                == 0
                                or track_id
                                not in plate_checked
                            )
                        ...
                        for box, cls, raw_id in zip(
                            ...
                            result.vehicle_frames_processed += 1

                            if (
                                track_id
                                in plate_predictions
                            ):

                                plates, vehicle_coords = (
                                    plate_predictions[
                                        track_id
                                    ]
                                )
                                ...
                                if (
                                    plates.boxes is not None
                                    and len(plates.boxes)
                                ):

                                    best = int(
                                        plates.boxes.conf.argmax().item()
                                    )
                                    ...
                                    result.plates_detected += 1

                                    if not record.plate_detected:
                                        result.vehicles_with_plate += 1

                                    record.plate_detected = True

                                    if (
                                        plate_crop.shape[0] >= 18
                                        and plate_crop.shape[1] >= 30
                                    ):
                                        ...
                                        if (
                                            ...
                                            if (
                                                self.settings.save_plate_crops
                                            ):
                ...
                for (
                    ...
                    cv2.putText(
                        annotated,
                        f"{video_path.stem}-V{track_id} F{frame_no}",
                        (
                            x1,
                            max(22, y1 - 8),
                        ),
        ...
        for track_id, record in tracks.items():
            ...
            # Use up to 10 best distinct plate observations.
            # This gives OCR more opportunities to read a clear plate.
            selected = sorted(
            ...
            record.number_of_plate_observations = len(
                plate_observations[track_id]
            )

            if selected:

                best_frame, best_crop, _best_conf, _best_quality = (
                    selected[0]
                )

                record.best_plate_width = (
                    best_crop.shape[1]
                )

                record.best_plate_height = (
                    best_crop.shape[0]
                )

            if self.ocr:

                record.ocr_backend = type(
                    self.ocr
                ).__name__
                ...
                for (
                    ...
                    frame_candidates = self._ocr_cache.get(
                        cache_key
                    )
            ...
            if observations[track_id]:
                ...
                confidence = min(
                    ...
                    * 0.20
                    + consensus
                    * 0.20
                    + indian_plate_score(text)
                    * 0.15,
                )

                # Accept a valid plate from one strong frame
                # instead of requiring two separate frames.
                if (
                    distinct_frames >= 1
                    and confidence
                    >= self.settings.min_ocr_confidence
                    and indian_plate_score(text) > 0
                ):

                    record.recognized_plate = text
                ...
                record.plate_detected
                and record.status != "OCR Unavailable"
            ):

                record.status = "OCR Unreadable"

        result.records = list(
            tracks.values()
        )

        result.vehicles_detected = len(
            tracks
        )

        result.tracked_vehicles = len(
            tracks
        )

        result.recognized_plates = sum(
            r.recognized_plate != "UNKNOWN"
            for r in tracks.values()
        )

        result.unreadable_plates = sum(
            r.plate_detected
            and r.recognized_plate == "UNKNOWN"
            for r in tracks.values()
        )
...
def save_result(
    ...
    def row(record):
        ...
        return {
            ...
            "plate_bbox": record.plate_bbox or "",
            "best_frame": record.best_frame,
            "crop_width": record.best_plate_width,
            "crop_height": record.best_plate_height,
            "ocr_backend": record.ocr_backend,
            "ocr_attempts": record.ocr_attempts,
            "distinct_frames": record.distinct_frames_used,
            "raw_candidates": record.ocr_candidates,
            "final_plate": record.recognized_plate,
